...
namespace MiniGamesSystem
{
    public class Dummy
    {
        ...
        {
            get => (PlayerMovementState)Player.ReferenceHub.animationController.Network_curMoveState;
            set => Player.ReferenceHub.animationController.Network_curMoveState = (byte)value;
        }
        ...
        private IEnumerator<float> Update()
        {
            for (; ; )
            {
                ...
                {
                    Player.ReferenceHub.animationController.Networkspeed = new Vector2(0f, 0f);
                    continue;
                }
                ...
                switch (Direction)
                {
                    case MovementDirection.Forward:
                        Player.ReferenceHub.animationController.Networkspeed = new Vector2(speed, 0f);
                        var pos = Position + Player.CameraTransform.forward / 10 * speed;

                        if (!Physics.Linecast(Position, pos, Player.ReferenceHub.playerMovementSync.CollidableSurfaces))
                            Player.ReferenceHub.playerMovementSync.OverridePosition(pos, 0f, true);
                        else wall = true;
                        break;
                    ...
                    case MovementDirection.Right:
                        Player.ReferenceHub.animationController.Networkspeed = new Vector2(0f, speed);
        ...
        public void Despawn()
        {
            ...
            Extensions.DumHubs.Remove(this.Player);
        }
        ...
        {
            NetworkServer.Spawn(GameObject);
            Extensions.DumHubs.Add(this.Player, new List<GameObject>());
        }
